...
namespace Stateless
{
    public partial class StateMachine<TState, TTrigger, TContext>
    {
        ...
        {
            var representativeState = GetRepresentation(context.CurrentState);
        ...
        async Task InternalFireOneAsync(TTrigger trigger, TContext context, params object[] args)
        {
            ...
            var source = context.CurrentState;
            ...
            switch (result.Handler)
            {
                ...
                default:
                    throw new InvalidOperationException("CurrentState machine configuration incorrect, no handler for trigger.");
            }
        }
        ...
        private async Task HandleReentryTriggerAsync(object[] args, StateRepresentation representativeState, Transition transition)
        {
            ...
            transition.Context.CurrentState = representation.UnderlyingState;
        }
        ...
        private async Task HandleTransitioningTriggerAsync(object[] args, StateRepresentation representativeState, Transition transition)
        {
            ...
            transition.Context.CurrentState = transition.Destination;
            var newRepresentation = GetRepresentation(transition.Destination);
            ...
            if (!representation.UnderlyingState.Equals(transition.Context.CurrentState))
            {
                // The state has been changed after entering the state, must update current state to new one
                transition.Context.CurrentState = representation.UnderlyingState;
            }

            await _onTransitionCompletedEvent.InvokeAsync(CreateTransition(transition.Source, transition.Context.CurrentState, transition.Trigger, transition.Context, transition.Parameters), RetainSynchronizationContext);
        }
        ...
        private async Task<StateRepresentation> EnterStateAsync(StateRepresentation representation, Transition transition, object[] args)
        {
            ...
            if (FiringMode.Immediate.Equals(_firingMode) && !transition.Context.CurrentState.Equals(transition.Destination))
            {
                // This can happen if triggers are fired in OnEntry
                // Must update current representation with updated State
                representation = GetRepresentation(transition.Context.CurrentState);
                transition = CreateTransition(transition.Source, transition.Context.CurrentState, transition.Trigger, transition.Context, args);
            }
