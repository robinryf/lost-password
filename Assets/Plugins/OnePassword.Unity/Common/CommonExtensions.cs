...
namespace OnePassword.Common
{
	...
	internal static class CommonExtensions
	{
			...
			where TField : Enum
		{
			...
			if (attributes.Length == 0)
				throw new NotImplementedException(
					$"The field has not been annotated with a {nameof(EnumMemberAttribute)}.");

			var value = attributes[0].Value;
			if (value is null)
				throw new NotImplementedException(
					$"{nameof(EnumMemberAttribute)}.{nameof(EnumMemberAttribute.Value)} has not been set for this field.");
		...
		internal static string ToCommaSeparated<TField>(this IEnumerable<TField> fields,
			bool replaceUnderscoresWithSpaces = false)
		...
		internal static string ToCommaSeparated(this IEnumerable<string> items,
			bool replaceUnderscoresWithSpaces = false)
		{
