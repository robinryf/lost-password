...
namespace OnePassword
{
	...
	public sealed partial class OnePasswordManager
	{
		...
		/// <exception cref="ArgumentException">Thrown when there is an invalid argument.</exception>
		public VaultDetails CreateVault(string name, string? description = null, VaultIcon icon = VaultIcon.Default,
			bool? allowAdminsToManage = null)
		{
		...
		public void EditVault(IVault vault, string? name = null, string? description = null,
			VaultIcon icon = VaultIcon.Default, bool? travelMode = null)
		{
			if (vault.Id.Length == 0)
				throw new ArgumentException($"{nameof(vault.Id)} cannot be empty.", nameof(vault));
			...
			if (name is null && description is null && icon is VaultIcon.Default or VaultIcon.Unknown &&
			    travelMode is null)
		...
		public void GrantPermissions(IVault vault, IGroup group, IReadOnlyCollection<VaultPermission> permissions)
		{
			...
			var command =
				$"vault group grant --vault {vault.Id} --group {group.Id} --permissions \"{permissions.ToCommaSeparated()}\"";
			Op(command);
		}
		...
		public void GrantPermissions(IVault vault, IUser user, IReadOnlyCollection<VaultPermission> permissions)
		{
			...
			var command =
				$"vault user grant --vault {vault.Id} --user {user.Id} --permissions \"{permissions.ToCommaSeparated()}\"";
			Op(command);
		}
		...
		public void RevokePermissions(IVault vault, IGroup group, IReadOnlyCollection<VaultPermission> permissions)
		{
			...
			var command =
				$"vault user revoke --vault {vault.Id} --group {group.Id} --permissions \"{permissions.ToCommaSeparated()}\"";
			Op(command);
		}
		...
		public void RevokePermissions(IVault vault, IUser user, IReadOnlyCollection<VaultPermission> permissions)
		{
			...
			var command =
				$"vault user revoke --vault {vault.Id} --user {user.Id} --permissions \"{permissions.ToCommaSeparated()}\"";
			Op(command);
		}
		...
		/// <exception cref="ArgumentException">Thrown when there is an invalid argument.</exception>
		public IReadOnlyList<Vault> GetVaults(IGroup group)
		{
			if (group.Id.Length == 0)
				throw new ArgumentException($"{nameof(group.Id)} cannot be empty.", nameof(group));
		...
		/// <exception cref="ArgumentException">Thrown when there is an invalid argument.</exception>
		public IReadOnlyList<Vault> GetVaults(IUser user)
		{
			if (user.Id.Length == 0)
				throw new ArgumentException($"{nameof(user.Id)} cannot be empty.", nameof(user));
